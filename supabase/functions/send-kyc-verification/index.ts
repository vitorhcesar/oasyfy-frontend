...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'verify') {
      // Need to re-read body - actually let's handle it differently
      return new Response(JSON.stringify({ error: 'Use action=send' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    await client.send({
      ...
      to: email,
      subject: 'Confirmação de e-mail - Oasyfy',
      content: `Seu código de verificação é: ${code}. Válido por 10 minutos.`,
      html: `
        <!DOCTYPE html>
        <html>
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 460px; background: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
                <tr>
                  <td style="background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); padding: 36px 32px 28px; text-align: center;">
                    ...
                    <h1 style="color: #ffffff; font-size: 22px; font-weight: 700; margin: 0; letter-spacing: -0.3px;">Confirme seu e-mail</h1>
                    <p style="color: rgba(255,255,255,0.8); font-size: 13px; margin: 8px 0 0; font-weight: 400;">Insira o código abaixo para verificar seu e-mail</p>
                  </td>
                </tr>
                <tr>
                ...
                <tr>
                  <td style="padding: 0 32px;"><div style="height: 1px; background: #f1f5f9;"></div></td>
                </tr>
                <tr>
    ...
    console.error('send-kyc-verification error:', err)
